...
int index_save(const Index *index) {
    // Sort a heap-allocated copy (Index struct is too large for the stack)
...
int index_add(Index *index, const char *path) {
    ...
    void *contents = malloc((size_t)fsize);
    if (!contents) { fclose(f); return -1; }
    fread(contents, 1, (size_t)fsize, f);
    fclose(f);

    // Write blob
    ObjectID blob_id;
    if (object_write(OBJ_BLOB, contents, (size_t)fsize, &blob_id) != 0) {
